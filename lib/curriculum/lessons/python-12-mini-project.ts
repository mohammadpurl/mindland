/**
 * سازگاری UI برای PY-12 — پروژه ترکیبی کوچک
 */



import { getPython12MiniProjectScenario } from '@/lib/curriculum/scenarios/loadScenario'

import fullLesson from '@/lib/curriculum/scenarios/python-12-mini-project/python-12-mini-project-full.json'

import type { ResolvedScenarioStep } from '@/lib/curriculum/scenarios/types'



export type Py12Band = 'A' | 'B' | 'both'



export type Py12StepId =
...
export interface Python12MiniProjectLesson {
...
function bandOf(step: ResolvedScenarioStep): Py12Band {
...
function mapStep(step: ResolvedScenarioStep): Py12Step {

  const id = step.id as Py12StepId
...
export function getPython12MiniProjectLesson(): Python12MiniProjectLesson {

  const scenario = getPython12MiniProjectScenario()
  ...
  return {
    ...
    goals: [
      'منو و تابع و حلقه را ترکیب کند',
      'input در پروژه استفاده کند',
      'برنامهٔ کوچک قابل اجرا بسازد',
    ],
