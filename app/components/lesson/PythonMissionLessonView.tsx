...
import {
  PythonCodeRunner,
  type RunnerLessonProfile,
} from '@/app/components/lesson/PythonCodeRunner'
import { markLessonCompleted } from '@/lib/lessonProgress'

export interface MissionLessonStep {
  id: string
  title: string
  band: 'A' | 'B' | 'both'
  stageMission?: string
...
export interface MissionLessonData {
  id: RunnerLessonProfile
  code: string
  title: string
  subtitle: string
  duration: string
  steps: MissionLessonStep[]
}

function avatarLinesOf(step: MissionLessonStep): string[] {
  const fromDialogue =
    step.dialogueLines
      ...
      <div className="lesson-classroom__grid">
        ...
        <section className="lesson-classroom__stage min-h-0" aria-label={step.title}>
          <div className="lesson-classroom__stage-inner !items-stretch !p-3 md:!p-4">
            ...
            {showRunner ? (
              <PythonCodeRunner
                key={`${lesson.id}-${stepId}`}
                mode={stepId === 'demo' ? 'demo' : 'interactive'}
                band={bandFocus}
                lessonProfile={lesson.id}
...
export function PythonMissionLessonView({
  lang,
  lesson,
  crumbLabel,
}: {
  ...
  return (
    <ChatProvider>
      ...
      <div className="lesson-classroom-shell px-3 py-4 md:px-6 md:py-6">
        ...
        <MissionClassroom lang={lang} lesson={lesson} />
      </div>
    </ChatProvider>
  )
}
